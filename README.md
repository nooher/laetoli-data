...
- **Storage service** (`storage/`, Node/Express, `:9998`) — sovereign object storage. Buckets + object metadata in Postgres (`storage` schema, RLS), bytes on a **filesystem volume** (no MinIO/S3 — lighter on a Pi). Public & private buckets, owner-scoped writes, time-limited **signed URLs**, and **on-the-fly image transforms** (`?width=…&height=…&format=webp&quality=…`, sharp-backed, disk-cached — see `docs/STORAGE-TRANSFORMS.md`).
