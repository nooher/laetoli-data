...
//
// The realtime service holds ONE dedicated pg client that runs
// `LISTEN laetoli_realtime` and forwards each NOTIFY payload to a callback.
// Production uses `createPgListener`; tests inject a `FakeListener` so the hub
// and server can be exercised without a live database or real sockets.
 ...
 */
export function createPgListener(config: RealtimeConfig): NotificationSource {
  ...
  async function connect(): Promise<void> {
    ...
    c.on('error', (err) => {
      ...
      healthy = false;
      // Let the 'end'/reconnect path handle re-establishing.
      scheduleReconnect();
    });
    try {
      await c.connect();
      ...
      console.log(`[realtime] LISTEN ${config.channel} established`);
    } catch (e) {
      healthy = false;
      console.error('[realtime] pg connect failed:', e instanceof Error ? e.message : String(e));
      scheduleReconnect();
    }
  }
